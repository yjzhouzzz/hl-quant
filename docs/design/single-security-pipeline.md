...
| 数据源 | 腾讯财经前复权日线（免认证、分页拉多年） |
| 区间 | 2018-01-01 ~ 2026-02-28（约 8 年，覆盖多轮牛熊） |
...
>
> 数据接口为非官方公开接口，单次最多稳定返回约 800 条，`load_prices` 用
> 「end 游标往前翻页」拼出多年区间；接口可能限频/改格式，失败即停下报告。
...
- 最近约 20%（多年区间下约 1.5 年）留作 **run-once** 样本外确认。
- 多年区间下 holdout 交易样本较充足（十余笔），可作**方向性确认**，但仍
  **不作硬门槛**（run-once，避免反复看导致选择偏差）。每轮反过拟合把关靠：
