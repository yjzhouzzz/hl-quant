...
本文件当前状态 = HL 第 2 轮接受的候选：
    ...
    评估口径已于 2026-07 一次性升级为「次日开盘成交 + 100 股整手 + 滑点」，
    对齐聚宽实盘（见 backtest.py 与 docs/guide/common-pitfalls.md），此后分数
    重锚：纯 10/20 基线 score −0.1755（总收益 −27.11%，回撤 42.34%，57 笔）
    → 叠加趋势过滤 score +0.0609（总收益 +0.30%，回撤 35.37%，39 笔）。趋势
    过滤在新口径下仍严格优于基线，说明它不是「收盘成交乐观偏差」的假象。
